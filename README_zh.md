...
| `token` | GitHub Token（私有仓库需要，也可通过 `GITHUB_TOKEN` 环境变量设置） |
| `format` | `md` 或 `html` |
| `severity` | 最低严重级别过滤 |
| `outputPath` | 自定义输出文件路径 |

返回值：报告文件路径 + 结构化漏洞详情（CVSS 分数、依赖链、修复建议）。

> **Token 是可选的。** 本地项目审计完全不需要 Token。远程公开仓库无需 Token 即可使用（每小时 60 次请求）。只有审计私有仓库时才需要配置 GitHub Token。

## 忽略机制

在项目根目录创建 `.audit-mcp-cli-ignore.json` 来忽略已确认接受的漏洞：
